...
from Pbxbot.functions.images import get_wallpapers, deep_fry
from Pbxbot.functions.tools import runcmd
...
async def searchImage(_, message: Message):
    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Provide a search query.")
    ...
    if to_send:
        ...
        await Pbxbot.delete(Pbx, "Uploaded!")
    else:
        await Pbxbot.delete(Pbx, "No images found.")
...
async def searchWallpaper(_, message: Message):
    ...
    if not access:
        return await Pbxbot.delete(Pbx, "Unsplash API not found.")
    ...
    if limit > 30:
        return await Pbxbot.delete(Pbx, "Limit should be less than 30.")
    elif limit < 1:
        return await Pbxbot.delete(Pbx, "Limit should be greater than 0.")
...
async def glitcher(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await Pbxbot.delete(message, "Reply to a media message to glitch it.")

    Pbx = await Pbxbot.edit(message, "Glitching...")
    ...
    if not 0 < intensity < 9:
        await Pbx.edit("intensity should be between 1 and 8... now glitching at 8")
    ...
    if media and media not in [
        ...
        return await Pbxbot.delete(Pbx, "Only media messages are supported.")
    ...
    if dwl_path.endswith(".tgs"):
        ...
        if not os.path.lexists(glitch_img):
            return await Pbxbot.error(Pbx, f"`{stdout}`\n`{stderr}`")
    elif dwl_path.endswith(".webp"):
        os.rename(dwl_path, glitch_img)
        if not os.path.lexists(glitch_img):
            return await Pbxbot.error(Pbx, "File not found.")
    ...
    await Pbxbot.delete(Pbx, f"Glitched at intensity {intensity}")
...
async def deepfry(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await Pbxbot.delete(message, "Reply to a photo to deepfry it.")
    ...
    Pbx = await Pbxbot.edit(message, "Deepfrying...")
    ...
    await Pbx.reply_photo(fried)
    await Pbxbot.delete(Pbx, "Deepfried!")
